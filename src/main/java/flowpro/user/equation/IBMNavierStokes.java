 ...
 * @author ales
 */
public class IBMNavierStokes extends Aerodynamics {
    ...
    public void init(FlowProProperties props) throws IOException {
        ...
        xProfile = props.getDoubleArray("xProfile");
        yProfile = props.getDoubleArray("yProfile");
    }
    ...
    boolean inside(double[] X){
        ...
        for(int i = 0; i < xProfile.length; i++){
            int ip = i+1;
            if(ip == xProfile.length){
                ip = 0;
            }
            if((X[0] >= xProfile[i] && X[0] < xProfile[ip]) || (X[0] <= xProfile[i] && X[0] > xProfile[ip])){
                double xi = (X[0] - xProfile[i])/(xProfile[ip]-xProfile[i]);
                double y = (1-xi)*yProfile[i] + xi*yProfile[ip];
                if(y > X[1]){
                    top++;
                }else{
                    down++;
