 ...
 * @author ales
 */
public class SpalartAllmaras5 extends Aerodynamics {
    ...
    public double[] diffusiveFlux(double[] W, double[] dW, double[] n, ElementData elem) {
        ...
        for (int d = 0; d < dim; ++d) {
            ...
            flux[dim + 2] += 1 / (Re * sigma) * (1 + rho * vt) * vtDer[d] * n[d]; //???? (rho+vt)
        }
    ...
    public double[] sourceTerm(double[] W, double[] dW, ElementData elem) { // zdrojovy clen
        ...
        double Sh = 1 / Re * vt / (ka * ka * D * D) * fv2;
        double c2 = 0.7;
        double c3 = 0.9;
        double S = Om + Sh;
        if(Sh  < -c2*Om){
            S = Om + (Om*(c2*c2*Om + c3*Sh))/((c3-2*c2)*Om - Sh);
        }
