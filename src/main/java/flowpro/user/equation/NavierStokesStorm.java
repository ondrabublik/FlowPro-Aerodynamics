 ...
 * @author ales
 */
public class NavierStokesStorm extends Aerodynamics {
            ...
            case (Aerodynamics.BoundaryType.INLET):
                if (!isInletSupersonic) { // subsonic inlet
                    ...
                    if (p > pIn0) {
                        ...
                        double pInlet = pIn0*(1 + 0.02*(Math.sin(X[1]*t) + Math.sin(X[1]*2*t + Math.PI/2) + Math.sin(X[1]*4*t + Math.PI))/3);
                        mach = Math.sqrt((2 / (kapa - 1)) * (-1 + Math.pow(pInlet / p, (kapa - 1) / kapa)));
                    }
